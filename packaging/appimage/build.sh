#!/usr/bin/env bash
# Build a Calcumon AppImage from a git checkout.
#
# Usage:
#   cd packaging/appimage
#   ./build.sh
#
# Produces: Calcumon-$VERSION-$ARCH.AppImage in this directory
# (ARCH is uname -m: x86_64 or aarch64).
# Requires Ubuntu 26.04-class deps: meson, ninja, valac, pkg-config,
# GTK4/libadwaita/GtkSourceView 5/JavaScriptCoreGTK 6/json-glib,
...
OUTPUT_NAME="Calcumon-${VERSION}-${HOST_ARCH}.AppImage"
OUTPUT_PATH="${SCRIPT_DIR}/${OUTPUT_NAME}"

echo "Building ${OUTPUT_NAME} from ${REPO_ROOT}"

rm -rf "${APPDIR}" "${BUILDDIR}"
rm -f "${SCRIPT_DIR}/Calcumon-"*-"${HOST_ARCH}.AppImage"
mkdir -p "${APPDIR}"
...
DESKTOP_FILE="${APPDIR}/usr/share/applications/dev.calcumon.Calcumon.desktop"
ICON_SVG="${REPO_ROOT}/data/icons/hicolor/scalable/apps/dev.calcumon.Calcumon.svg"
ICON_FILE="${APPDIR}/usr/share/icons/hicolor/512x512/apps/dev.calcumon.Calcumon.png"
...
(
  ...
  env APPIMAGE_EXTRACT_AND_RUN=1 \
    ...
    "${LINUXDEPLOY}" \
    --appdir "${APPDIR}" \
    --executable "${APPDIR}/usr/bin/calcumon" \
